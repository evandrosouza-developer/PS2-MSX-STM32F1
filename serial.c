...
#define SERIAL_RING_BUFFER_SIZE (2 << SERIAL_RING_BUFFER_SIZE_POWER)
	...
	if (enable_xon_xoff)
	{
		if (*qty_in_buffer >= (uint16_t)X_OFF_TRIGGER)
		{
			...
			if (!xoff_condition)										//To send X_OFF only once
			{
				...
				// Enable Transmit request
				xonoff_sendnow = true;
				USART_CR1(USART2) |= USART_CR1_TXEIE; //Force Enable transmission request
			}
		}
...
// It returns true if there was room to put this on USART2 TX buffer.
...
// No return  and no blocking function if there is enough space on USART2 TX buffer, otherwise,
...
// It returns and no blocking function if there is enough space on USART2 TX buffer, otherwise,
...
{
	// Enable clocks for GPIO port A (for GPIO_USART2_TX in main) and USART2.
	...
	usart_set_baudrate(USART2, 38400); //115200  
...
void usart2_isr(void)
{
	...
	if(((USART_CR1(USART2)&USART_CR1_RXNEIE)!=0) && ((USART_SR(USART2)&USART_SR_RXNE)!=0))
	{
		...
		ring_put_ch(&rx_ring, (uint8_t)usart_recv(USART2));
	}
	...
	if(((USART_CR1(USART2) & USART_CR1_TXEIE)!=0) && ((USART_SR(USART2) & USART_SR_TXE)!=0))
	{
		...
		{ //  Put data into the transmit register.
			usart_send(USART2, (uint16_t) data);
		}
	}
}